...
###----------[ MASUK LISENSI ]---------- ###
def key():
    Console().print(Panel(f"{P2}silahkan masukan lisensi tools agar bisa masuk ke tools",width=60,style=f"{color_table}"))
    key = Console().input(f" {H2}•{P2} masukan lisensi : ")
    try:
        ses = requests.Session()
        asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyI3MDI1Mjg4MCIsIjR2cXpqby8xdU5uZnhIc3VDRlpaYzRTSDdXRjViM1lzZUN2bFRnSVAiXQ==&ProductId=23140&Key=%s&Sign=True"%(key)).json()['licenseKey']['key']
        open(".key.txt","w").write(key)
        Console().print(Panel(f"{H2}selamat lisensi yang anda masukan terdaftar ke server",width=60,style=f"{color_table}"))
        time.sleep(3)
        login()
    except KeyError:
        Console().print(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=60,style=f"{color_table}"))
        time.sleep(3)
        licen()
###----------[ CEK LISENSI ]---------- ###
def cek():
    try:
        ...
        licen()
    try:
        lopi = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyI3MDI1Mjg4MCIsIjR2cXpqby8xdU5uZnhIc3VDRlpaYzRTSDdXRjViM1lzZUN2bFRnSVAiXQ==&ProductId=23140&Key=%s"%(x)).json()['licenseKey']['key']
        login()
    except KeyError:
        Console().print(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=60,style=f"{color_table}"))
...
def asyn(idf,pwv):
    ...
    ua = rc(prem)
    emot = rc(["S","B","R"])
    prog.update(des,description=f" {K2}•{H2} ASYNC {SE}{SE}{idf} [bold blue]{loop}[bold white]/[bold blue]{len(id)} [bold green]OK : [bold green]{ok}  [bold white]-  [bold yellow]CP : [bold yellow]{cp}[white]")
    prog.advance(des)
    for pw in pwv:
        try:
            if 'ya' in ualuh: ua = ualu[0]
