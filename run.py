	...
	while True:
		...
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			if key in check:
				...
				if buy_key in [""]:pass
				jalan(f'\033]02;36\] • \033]00m\]Anda akan diarahkan ke whatsapp author');time.sleep(2)
...
def crackprod(idf,pwv):
	global loop,ok,cp
	prog.update(des,description=f" {K2}•{H2} Viper V.Prod {SE}{SE}{idf} [bold blue]{loop}[bold white]/[bold blue]{len(id2)} [bold green]OK : [bold green]{ok}  [bold white]-  [bold yellow]CP : [bold yellow]{cp}[white]")
	prog.advance(des)
