...
[{color_text}02{P2}]. {M2}EXIT{P2}""", width=60, title="MENU TANPA LOGIN", style=f"{color_panel}"))
    ...
    if HaHi in ["1", "01"]:
        try:
            file = console.input(f" {H2}• {P2}Masukkan nama file dump (cth: clara.txt): ")
            if not os.path.exists(file):
                console.print(f" {H2}• [red]File tidak ditemukan!")
                time.sleep(2)
                exit()
            ...
            if len(id_list) == 0:
                console.print(f" {H2}• [red]File kosong!")
                time.sleep(2)
                exit()

            # Masukkan ke variabel global id
            for user in id_list:
                try:
                    uid, name = user.split("|")
                    id.append({"uid": uid, "name": name})
                except ValueError:
                    continue  # Jika format salah, lewati

            console.print(Panel(f"{H2}Berhasil memuat {len(id)} ID dari file {file}", style=color_panel))
            setting()  # Masuk ke fungsi setting langsung
        except Exception as e:
            console.print(f"[red]Terjadi kesalahan: {e}")
            time.sleep(3)
            exit()
